...
function generate_comparison_report() {
    ...
    for model in "${MODELS[@]}"; do
        ...
        if [ -n "$latest_result" ] && [ -f "$latest_result" ]; then
            ...
            # Konwertuj kody wyjścia na symbole
            local queries_status=$([ "$queries_result" -eq 0 ] && echo "${GREEN}OK" || echo "${RED}BŁĄD")
            local correctness_status=$([ "$correctness_result" -eq 0 ] && echo "${GREEN}OK" || echo "${RED}BŁĄD")
            local performance_status=$([ "$performance_result" -eq 0 ] && echo "${GREEN}OK" || echo "${RED}BŁĄD")
            ...
            printf "${YELLOW}%-15s${NC} %-15s %-15s %-15s %-15s\n" "$model" "$queries_status" "$correctness_status" "$performance_status" "$total_score/3"
        else
            printf "${YELLOW}%-15s${NC} %-15s %-15s %-15s %-15s\n" "$model" "${YELLOW}BRAK" "${YELLOW}BRAK" "${YELLOW}BRAK" "0/3"
        fi
    done
