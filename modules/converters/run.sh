...
# Uniwersalny skrypt uruchamiający serwer dla modułów konwersji
...
echo "Uruchamianie serwera modułów konwersji..." | tee -a "$LOG_FILE"
echo "Logi będą zapisywane do: $LOG_FILE" | tee -a "$LOG_FILE"
...
if [ -f "requirements.txt" ]; then
    echo "Instalowanie wymagań z requirements.txt..." | tee -a "$LOG_FILE"
    pip3 install -r requirements.txt 2>&1 | tee -a "$LOG_FILE"
else
    echo "Plik requirements.txt nie istnieje, instalowanie Flask..." | tee -a "$LOG_FILE"
    pip3 install flask==2.0.1 werkzeug==2.0.0 2>&1 | tee -a "$LOG_FILE"
fi
